...
#73 no sig difference between intercept and slope - scenario D
dat_intslopesNS<- dat %>%
  filter (fm_diff_slope_p >0.05 & fm_diff_int_p > 0.05)
dim(dat_intslopesNS) 
...
# (41 + 82) = 124 out of (52 + 129) = 181; 124 out of 181
...
#8 out of 129 traits sig slope diff - scenario A
dat_slopes1 <-dat1 %>%
  filter (fm_diff_slope_p <= 0.05 & fm_diff_int_p > 0.05)
...
#48 out of 129 traits sig intercept diff  same slope - scenario B
dat_int1<- dat1 %>%
  filter (fm_diff_int_p <= 0.05 & fm_diff_slope_p >0.05)
dim(dat_int1) 
...
#52 out of 130 sig intercept and slope diff - scenario C
dat_intSlopes1<-dat1 %>%
  filter (fm_diff_int_p <= 0.05 & fm_diff_slope_p <= 0.05)
dim(dat_intSlopes1) 
...
#21 no sig difference between intercept and slope - scenario D
