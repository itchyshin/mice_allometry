---
title: "Supporting Information: 'Sex differences in allometry for phenotypic traits in mice indicate that females are not scaled males' "
...
dat_short %>% left_join(dat_category, by = ("parameter_name" = "parameter_name") ) %>% arrange(Category)  -> dat_short
...
# making character strings into factors
extra <- extra %>% mutate_if(is.character, as.factor) 
# visualizing
...
```{r}
# full dataset
# 249 out of 363 signficant differences in residual SDs
...
dat_short %>% group_by(parameter_group) %>% mutate(count = n()) -> dat_short
# 
...
# nesting data into a lot of data sets and apply p_mod function
n_dat_short2 <- dat_short2 %>% group_by(parameter_group) %>%  nest()
...
# merged data
m_dat_short2 <- n_dat_short2  %>% mutate(merged_p_comp = map_dbl(data, p_comp))
...
```{r, echo = FALSE}
fit_4b <- readRDS(here("data", "fit_4b.rds"))
...
p1 <- orchard_plot2(modelia, mod = "Int", xlab = "Absolute difference in standardized intercepts  (F-M)", angle = 45, point.size = point.size, N = dat$total_n, legend.on = TRUE, branch.size = branch.size,) +
...
p2 <- orchard_plot2(model1a, mod = "Category", xlab = "Absolute difference in standardized intercepts  (F-M)", angle = 45,  point.size = point.size, N = dat$total_n, legend.on = TRUE, branch.size = branch.size,) + 
...
f1 <- ggplot(data = dat) +
  ...
  annotate(geom="text", x = - 10, y = -0.5, label="r = 0.82 [0.76, 0.88]", size = 3)+
  theme_bw()  +
...
f2 <- ggplot(data = dat) +
  ...
  labs(color='Trait types', size = "Sample size (N)") +
  annotate(geom="text", x= -9.8, y = 0.5, label="r = 0.14 [-0.01., 0.30]", size = 3)+
  theme_bw()   +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_text(size=10)) + 
...
f3 <- ggplot(data = dat) +
  ...
  labs(color='Trait types', size = "Sample size (N)") +
  annotate(geom="text", x= - 9.5, y = 0.5, label="r = 0.07 [-0.08, 0.22]", size = 3)+
  theme_bw() +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_text(size=10)) +
  guides(size = "none") 
...
f4 <- ggplot(data = dat) +
  ...
  labs(color='Trait types',size = "Sample size (N)") +
  annotate(geom="text", x= -2.5, y = -6, label="r = 0.53 [0.41., 0.63]", size = 3)+
  theme_bw()   +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_text(size=10)) +
  guides(size = "none") #+
...
f5 <- ggplot(data = dat) +
  ...
  labs(color='Trait types', size = "Sample size (N)") +
  annotate(geom="text", x=  -2, y = -6, label="r = 0.37 [0.23, 0.41]", size = 3)+
  theme_bw() +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_text(size=10))+
  guides(col = "none", size = "none") +
  scale_size_continuous(breaks = c(200, 2000, 20000), guide = guide_legend()) +
...
f6 <- ggplot(data = dat) +
  ...
  labs(color='Trait types', size = "Sample size (N)") +
  annotate(geom="text", x= -0.25, y = - 5, label="r = 0.18 [0.02, 0.33]", size = 3)+
  theme_bw()  +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_text(size=10))+
  guides(col = "none") #+
  #scale_size_continuous(breaks = c(200, 2000, 20000), guide = guide_legend()) 
